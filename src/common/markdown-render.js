...
;(function() {
    ...
    */
    async function markdownRender(mdText, userprefs, marked, hljs) {
	let mathifyCache = [];
	let mathifyCachePlaceholderFor = id => "TO_BE_CHANGED_TO_MATH_"+(+id)+"_";
	function mathify(mathcode) {
	    mathifyCache.push(mathcode);
	    return mathifyCachePlaceholderFor(mathifyCache.length - 1);
	...
	for(let i in mathifyCache){
	    let mathcode = mathifyCache[i];
	    
	    let html = userprefs['math-value']
	    	.replace(/\{mathcode\}/ig, mathcode)
	    	.replace(/\{urlmathcode\}/ig, encodeURIComponent(mathcode));
	    let url = html.match(/src="([^"]*)"/)[1];
	    let base64 = await toDataURL(url);
	    html = html.replace(url, base64);
	    renderedMarkdown = renderedMarkdown.replace(mathifyCachePlaceholderFor(i), html);
	}
